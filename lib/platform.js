...
const normalizeCookies = (cookies, envCookies) => {
  if (envCookies) {
    return envCookies.split(';').map((c) => c.trim()).filter(Boolean);
  }
  if (Array.isArray(cookies)) {
    return cookies.map((c) => String(c).trim()).filter(Boolean);
  }
  return [];
};
...
class VerisurePlatform {
  ...
  async accessories(callback) {
    ...
    if (!this.hasCredentials()) {
      this.logger.error('Verisure: configure cookies or email and password.');
      return callback([]);
    }

const ContactSensor = require('./contactsensor');
const { createAccessory } = require('../testHelpers');

describe('ContactSensor', () => {
  const config = {
    device: {
      area: 'Front door',
      deviceLabel: 'DEFG 4567',
    },
    state: 'CLOSE',
  };
  const installation = {
    client: null,
    config: {
      alias: 'Home',
    },
  };

  const contactSensor = createAccessory(ContactSensor, {
    deviceConfig: config,
    installation,
  });

  it('setup name and value', () => {
    expect(contactSensor.name).toBe('Front door');
    expect(contactSensor.value).toBe(false);
  });

  it('resolves state', () => {
    expect(ContactSensor.resolveSensorState('OPEN')).toBe(true);
    expect(ContactSensor.resolveSensorState('CLOSE')).toBe(false);
    expect(ContactSensor.resolveSensorState('ASD')).toBe(true);
  });

  it('gets current state', async () => {
    expect.assertions(2);

    installation.client = jest.fn();
    installation.client.mockResolvedValueOnce({
      installation: {
        doorWindows: [{
          device: {
            deviceLabel: 'DEFG 4567',
          },
          state: 'OPEN',
        }],
      },
    });

    const value = await contactSensor.getCurrentSensorState();
    expect(value).toBe(true);
    expect(contactSensor.value).toBe(true);
  });

  it('exposes services', () => {
    const services = contactSensor.setupServices();
    expect(services.length).toBe(2);
    expect(Array.isArray(services)).toBe(true);
  });

  it('errors when sensor is missing from overview', async () => {
    installation.client = jest.fn().mockResolvedValueOnce({
      installation: { doorWindows: [] },
    });

    await expect(contactSensor.getCurrentSensorState()).rejects.toThrow(
      'Could not find contact sensor state for Front door.'
    );
  });
});

...
class DoorLock extends VerisureAccessory {
  ...
  resolveTargetLockState(verisureState) {
    const { LockTargetState } = this.homebridge.hap.Characteristic;

    if (verisureState === 'LOCKED') {
      return LockTargetState.SECURED;
    }
    if (verisureState === 'UNLOCKED') {
      return LockTargetState.UNSECURED;
    }

    throw new Error(`Unknown lock state from Verisure: ${verisureState}`);
  }
  ...
  async getTargetLockState() {
    ...
    return this.resolveTargetLockState(targetLockState);
  }
  ...
  async setTargetLockState(value) {
    ...
    try {
      ...
      const isAlreadyAtTargetStateError = error?.errors?.some(
        (entry) => entry?.data?.errorCode === 'VAL_00819'
      );
  ...
  setupServices() {
    ...
    const services = [this.ensureAccessoryInformation()];

    this.lockService = this.ensureService(Service.LockMechanism, this.name);
    ...
    if (this.platformConfig.showAutoLockSwitch) {
      this.autoLockService = this.ensureService(Service.Switch, this.switchName, this.switchName);
    ...
    if (this.platformConfig.showAudioSwitch) {
      this.audioService = this.ensureService(Service.Switch, this.audioName, this.audioName);
      this.audioService
        .getCharacteristic(Characteristic.On)
    ...
    this.pollCharacteristics.push({
      ...
      getter: async () => {
        ...
        if (this.targetLockState === 'NONE') {
          ...
          if (doorLock.currentLockState === 'LOCKED' || doorLock.currentLockState === 'UNLOCKED') {
